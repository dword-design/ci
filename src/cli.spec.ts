...
for (const [name, _testConfig] of Object.entries(tests)) {
  ...
  test(name, async ({}, testInfo) => {
    ...
    await fs.outputFile(pathLib.join(cwd, 'package.json'), JSON.stringify({ type: 'module' }))
    ...
    await execa('tsx', [resolver.resolve('./cli.ts'),
      'push-changed-files',
      remoteDir,
    ], { cwd: localDir })

    const output = await execaCommand('git log -n 1', { all: true, cwd: localDir })
